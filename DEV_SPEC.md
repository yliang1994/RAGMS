...
目标：将 PDF 等原始资料转化为可检索、可解释、可追踪的知识块。
...
- PDF 转换统一使用 MarkItDown，输出 Markdown 作为中间标准表示。
- 每个文档必须生成 `document_id`，每个 chunk 必须生成稳定的 `chunk_id`。
- chunk 内容由以下部分拼接而成：
  - 原始文本片段
  - 图片描述文本
  - 标题层级与章节信息
...
- chunk 元数据至少包含：
  ...
  - `ingestion_time`
  - `version`
- LLM 增强必须可关闭，并支持按配置分别控制：
  - `rewrite_for_retrieval`
  - `inject_metadata_text`
  - `image_captioning`
- 双路 Embedding 的含义是：
  - 检索向量：用于 Dense Retrieval
  - 可选增强向量接口预留：例如 query/document 双塔差异化模型或未来多场景向量
  - 首期至少落地一套 Dense Embedding，但架构要允许双路扩展
- 增量跳过规则：
  - 若 `source_path + sha256` 已存在且配置未要求强制重建，则跳过。
  - 若文档变化，则按 `document_id` 全量替换该文档相关 chunk。
